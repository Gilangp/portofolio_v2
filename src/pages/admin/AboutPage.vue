    ...
    <template v-else>
      ...
      <div v-if="showSchemaWarning" class="bg-amber-500/10 border border-amber-500/30 rounded-2xl p-5 space-y-3 transition-all">
        ...
        <div class="relative bg-slate-950 rounded-xl p-3 border border-slate-800 text-[11px] font-mono text-slate-300 max-h-40 overflow-y-auto">
          ...
          <pre class="whitespace-pre-wrap pr-28">ALTER TABLE site_settings ADD COLUMN IF NOT EXISTS about_photo_url text;
...
import { Save, Loader2, ImageIcon, UserSquare2, Sparkles, CheckCircle, AlertCircle, AlertTriangle, Copy } from 'lucide-vue-next'
...
const copySql = () => {
  const sql = `ALTER TABLE site_settings ADD COLUMN IF NOT EXISTS about_photo_url text;
ALTER TABLE site_settings ADD COLUMN IF NOT EXISTS about_title_en text;
ALTER TABLE site_settings ADD COLUMN IF NOT EXISTS about_title_id text;
ALTER TABLE site_settings ADD COLUMN IF NOT EXISTS about_bio_en text;
ALTER TABLE site_settings ADD COLUMN IF NOT EXISTS about_bio_id text;
...
const save = async () => {
  ...
  if (error && strippedColumns === 0) {
    ...
    showToast('success', 'Perubahan About berhasil disimpan ke database Supabase!')
  }
