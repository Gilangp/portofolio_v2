    ...
    <template v-else>
      ...
      <div v-if="showSchemaWarning" class="bg-amber-500/10 border border-amber-500/30 rounded-2xl p-5 space-y-3 transition-all">
        ...
        <div class="relative bg-slate-950 rounded-xl p-3 border border-slate-800 text-[11px] font-mono text-slate-300 max-h-40 overflow-y-auto">
          ...
          <pre class="whitespace-pre-wrap pr-28">ALTER TABLE site_settings ADD COLUMN IF NOT EXISTS contact_email text;
...
import { Save, Loader2, Phone, Clock, CheckCircle, AlertCircle, AlertTriangle, Copy } from 'lucide-vue-next'
...
const copySql = () => {
  const sql = `ALTER TABLE site_settings ADD COLUMN IF NOT EXISTS contact_email text;
ALTER TABLE site_settings ADD COLUMN IF NOT EXISTS contact_location text;
ALTER TABLE site_settings ADD COLUMN IF NOT EXISTS contact_response_en text;
ALTER TABLE site_settings ADD COLUMN IF NOT EXISTS contact_response_id text;
...
const save = async () => {
  ...
  if (error && strippedColumns === 0) {
    ...
    showToast('success', 'Informasi Kontak berhasil disimpan ke database Supabase!')
  }
